...
export default async function handler(req, res) {
  ...
  try {
    let row = null;
    let err = null;
    let r = await supabaseAdmin.from("hotel_information").select("hotel_name, hotel_description, hotel_logo_url, hotel_logo_footter_url, hotel_bg_url").limit(1).maybeSingle();
    err = r.error;
    row = r.data;
    ...
    return res.status(200).json({
      data: { hotelName: "Neatly Hotel", hotelDescription: null, hotelLogoUrl: null, hotelLogoFooterUrl: null, hotelBgUrl: null },
    });
  }
}
