...
import {
  ...
  InputGroupButton,
  InputGroupInput,
} from "@/components/ui/CountrySelector/input-group"
  ...
  return (
    <ComboboxPrimitive.Trigger
      ...
      <ChevronDownIcon
        ...
        className="text-gray-600 pointer-events-none size-4" />
    </ComboboxPrimitive.Trigger>
  );
}
...
function ComboboxItem({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "data-highlighted:bg-gray-200 data-highlighted:text-accent-foreground relative flex w-full cursor-default items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}>
      {children}
      <ComboboxPrimitive.ItemIndicator
        ...
        <CheckIcon className="pointer-events-none text-green-600 size-4 pointer-coarse:size-5" />
      </ComboboxPrimitive.ItemIndicator>
    </ComboboxPrimitive.Item> 
  );
}
